...
namespace ESL.Core.Models
{
    ...
    public partial record SOC //: LogEvent
    {
        ...
        [Column("LIMITATIONS", TypeName = "VARCHAR2")]
        ...
        [Column("MODIFYFLAGE", TypeName = "VARCHAR2")]
        ...
        public string EventTrail
        {
            get
            {
                ...
                if (!String.IsNullOrEmpty(ReleaseType))
                {
                    ...
                    {
                        case "Full Release":
                            _EventTrail += $"Full Released by: ReleasedBy_Name{_CrLf}Full Released to: ReleasedTo_Employee.FullName{_CrLf}";
                            if (ReleasedDate.HasValue) _EventTrail += $"Full Released Dt/Tm: {ReleasedDate.Value.ToString("MM/dd/yyyy")} {ReleasedTime}{_CrLf}";
                            break;

                        case "Test Release":
                            _EventTrail += $"Test Released by: ReleasedBy_Name{_CrLf}Test Released to: ReleasedTo_Name{_CrLf}";
                            if (ReleasedDate.HasValue) _EventTrail += $"Test Released Dt/Tm: {ReleasedDate.Value.ToString("MM/dd/yyyy")} {ReleasedTime}{_CrLf}";

                            break;

                        case "Transfer":
                            _EventTrail += $"Released by: ReleasedBy_Name{_CrLf} + Issud to: ReleasedTo_Name{_CrLf}";
